...
def main(event, context):
    ...
    try:
        ...
        logging.exception(e)
        request_error = 'Something went wrong while making a request to OpenAI API'
        failed_response = dict(body=dict(description=request_error),
                               statusCode=response.status_code)
        return failed_response
    content = response_json.get('choices')[0]['message']['content']
    proxy_response = dict(body=dict(description=description, content=content))
